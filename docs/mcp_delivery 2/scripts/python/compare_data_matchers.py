...
def demo_cases(path: Path) -> Iterable[Dict[str, Any]]:
    ...
    for index, entry in enumerate(value.get("queries") or []):
        runs = entry.get("runs") or []
        full = ((runs[0].get("full_result") or {}) if runs else {})
        intent = full.get("intent") or {"query_text": entry.get("query") or ""}
        matched = ((full.get("result") or {}).get("matched_pipelines") or [])
        pipeline_ids = [item.get("pipeline_id") for item in matched if item.get("pipeline_id")]
        yield _case(f"demo-{index + 1:02d}", "demo_queries", intent, pipeline_ids)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    cases = [
        ...
        *demo_cases(PACKAGE_ROOT / "docs/demo_queries.json"),
