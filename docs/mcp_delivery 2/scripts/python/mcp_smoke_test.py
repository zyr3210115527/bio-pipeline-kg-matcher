...
APP_ROOT = PACKAGE_ROOT / "app" if (PACKAGE_ROOT / "app/server.py").exists() else PACKAGE_ROOT
...
def live_exchange(
    ...
    completed = subprocess.run(
        [sys.executable, str(APP_ROOT / "server.py")],
        cwd=APP_ROOT,
        env=env,
...
def checks() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "assert": lambda response: (value(response) or {}).get("schema_version") == "tool-chain/v1",
        },
        {
        ...
        },
        {
            ...
            "assert": lambda response: (
                (value(response) or {}).get("selection_status") in {"no_match", "missing_assets"}
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--cassette", default=str(PACKAGE_ROOT / "mcp_smoke_cassette.json"))
    parser.add_argument("--output", default=str(PACKAGE_ROOT / "mcp_smoke_result.json"))
