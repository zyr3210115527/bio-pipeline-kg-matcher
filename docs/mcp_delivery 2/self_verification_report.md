...
- Unit suite: 81 tests OK; 3 pre-existing real-integration tests skipped.
...
| Task | Result | Timing |
|---|---|---:|
| RNA-seq standard plan | ready; exact 7-step recipe ending in MultiQC | 2,058.6 ms cold |
| Legacy standard shape | single `rnaseq_singletask` node | 154.2 ms hot |
| Single-sample WES validation | valid | 0.3 ms hot |
| Custom-step data availability | available; required role `fastq_file` | 188.8 ms hot |
| MAF capability query | information; 4 pipelines | 0.6 ms hot |
