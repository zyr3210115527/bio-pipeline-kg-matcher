...
DEFAULTS = {
    ...
    "DATAGRAPH_EXPECTATIONS_PATH": "config/unified_graph_expectations.json",
...
def initialize_runtime(
    ...
    if not target.get("DATAGRAPH_SNAPSHOT_ID"):
        expectation_path = Path(target["DATAGRAPH_EXPECTATIONS_PATH"])
        if not expectation_path.is_absolute():
            expectation_path = HERE / expectation_path
        try:
            payload = json.loads(expectation_path.read_text(encoding="utf-8"))
            snapshot_id = ((payload.get("data_graph") or {}).get("snapshot_id") or "").strip()
            if snapshot_id:
                target["DATAGRAPH_SNAPSHOT_ID"] = snapshot_id
